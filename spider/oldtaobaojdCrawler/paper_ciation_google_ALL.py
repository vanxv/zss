...
def main():
    ...
    k_time_sleep = 70
    k_time_sleep1 = 70
    time_reserve = 1.5*24*3600                # every 1.5 days sleep for 1.5 hour
    #-----------------------------------------------------------------------------------------
    # The path for saving (1) webpage; (2) information of publication; (3) download url
    download_page_fold_original = r"E:\\Z_Project\\Data_Research\\Publication_Citation\\paper_cita_google\Webpage\\"               # the fold saving all the dowloaded webpage
    pubcita_infor_file_fold_original = r"E:\\Z_Project\\Data_Research\\Publication_Citation\\paper_cita_google\\Element Infor\\"   # the fold saving all the publication information, here in csv format
    download_url_list_fold_original = r"E:\\Z_Project\\Data_Research\\Publication_Citation\\paper_cita_google\\Download List\\"     # the fold saving all the webpage list of every reseacher, which is generated after downloading a webpage
    download_webpage_open_fold_original = r"file:///E:/Z_Project\Data_Research/Publication_Citation/paper_cita_google/Webpage/"    # the original url for opening the dowloaded webpage
    #-------------------------------------------------------------------------------------------------

    #-------------------------------------------------------------------------------------------------------
    time_create= time.strftime('%Y%m%d',time.localtime(time.time()))
    pubcita_infor_file_fold = pubcita_infor_file_fold_original + time_create + r"\\"        # fold for saving download url list
    mkdir(pubcita_infor_file_fold)
    #-------------------------------------------------------------------------------------------------------
 

    download_url_list_fold = r"E:\\Z_Project\\Data_Research\\Publication_Citation\\paper_cita_google\\Download List\\20160308\\"
    uiniversity_name = os.listdir(download_url_list_fold)
    print uiniversity_name
    print len(uiniversity_name)
    
    k_page = 1
    ind_end = len(uiniversity_name)

    txt_name = uiniversity_name[0]
    print txt_name
    university = txt_name
    #------------------------------------------------------------------------------------------------------
    page_name_file_path =     download_url_list_fold + university  
    print page_name_file_path 
    #-------------------------------------------------------------------------------------------------------
    time_create= time.strftime('%Y%m%d',time.localtime(time.time()))
    download_page_fold  = download_page_fold_original  + university.replace('.txt','') + r"_" +time_create + r"\\"  # fold for saving download url list     
    mkdir(download_page_fold)
    print 
    #------------------------------------------------------------------------------------------------------- 
        
    #-------------------------------------------------------------------------------------------------------
    total_element_infor_list_path = pubcita_infor_file_fold + university        # txt file for saving download url list
    print total_element_infor_list_path
    list_file = open(total_element_infor_list_path,'w') # create a file saving the download url

    #-------------------------------------------------------------------------------------------------------
    # Extract the information of papers
    f_txt = open(page_name_file_path )             #
    

    #-----------------------------------------
    # find out the total number of links
#     line = f_txt.readline()             #  readline() 
#     i_line = 1
#     while line:
#         i_line = i_line + 1
#         line = f_txt.readline()
#         print i_line     # the total number of links
#         print line

    #------------------------------------------------------------
    # Generate a list including randomly and not repeat number from 1 to total number and write it into a list
#         i_line_path = download_url_list_fold + 'ALL_I_LINE.txt'           # txt file including the random number
#         list_file = open(total_element_infor_list_path,'w')
#         I_Line = random.sample(range(126279), 126279)
#         print I_Line
#         for i_line in range(1,126279):
#             print I_Line[i_line]
#             list_file = open(i_line_path,'a')
#             list_file.writelines(str(I_Line[i_line]) +'\n')                  #'\n' create a new line, Note that on windows, it is '\n', on Mac it is '\r', on Unix, it is '\n'
#             list_file.close()
        
        
    #----------------------------------------------------------------------------------------
    i_line_path = download_url_list_fold + 'ALL_I_LINE.txt'           # txt file including the random number
    i_txt = open(i_line_path )             #
    i_line = i_txt.readline()             #  readline() 
     
    k_page = 1
    begin_page = 27778
    while i_line:
        if k_page>=begin_page:
            start = time.clock()
            if len(i_line.strip('\n')) > 0:           
                i_num = int(i_line.strip('\n'))
                ...
                paper_year = re.findall(u'Year:\[(.*?)\]', line, re.S)
                paper_year = paper_year[0]
                 
                paper_title = re.findall(u'Title:\[(.*?)\]', line, re.S)
                paper_title = paper_title[0]
                paper_title = paper_title[0:40]
                 
                paper_url = re.findall(u'URL:\[(.*?)\]', line, re.S)
                ...
                browser_web,url_result = OpenUrl(paper_url,k_time_sleep,f_thr)
                if url_result == True:
                    unicodePage = browser_web.decode("utf-8")
                    page_saving_path = download_page_fold + paper_title +'-'+ time_open + '.html'
                    try: 
                        open(page_saving_path,'w').write(browser_web)
                    except:
                        print 'Can not save the page ' + page_saving_path
                   
                    #---------------------------------------------------------------------------------------------------------------------
                    # Find the information of video
                    # Find every block saving the information of a video
                    #---------------------------------------------------------------------------------------------------------
                    try:
                    # re.S is the matching form, which means '.' can match  newline character
                        myItems0 = re.findall(u'(Cited by.*?)Cited by', unicodePage, re.S)   # find out the main block including the neccessarly information.
                           
                        item_t = re.findall(u'Cited by (\d*?)</a>', myItems0[0], re.S) 
                        cita_total = item_t[0]
                       
                           
                    #    myItems1 =  re.findall(u'<span class="gsc_g_t" style="left\:\d*?px">(\d*?)</span>', myItems0[0], re.S)   # citation year
                        myItems1 =  re.findall(u'amp;as_yhi=(\d*?)" class="gsc_g_a"', myItems0[0], re.S)   # citation year
                        myItems2 = re.findall(u'<span class="gsc_g_al">(\d*?)</span>', myItems0[0], re.S)   # citation number
                       
                           
                        cita_infor = 'Cita Infor:'
                        if abs(int(myItems1[len(myItems1)-1]) - now.year) < 0.1:
                            for i_year in range(0,len(myItems1)-1):
                                cita_infor = cita_infor + '[' + myItems1[i_year]  + ';' + myItems2[i_year] + '];'
                                               
             
                            #------------------------------------------------------
                            # write the information          
                            element_infor = 'University:[' + university + ']; '  + 'Title:[' + paper_title + ']; ' + 'Year:[' + paper_year + '];' + '\n'
                            element_infor = element_infor   +  'Open Time:[' + time_open + ';' + myItems2[len(myItems1)-1]+ ']; ' + 'URL:[' + paper_url + ']; ' + '\n'
                            element_infor = element_infor + cita_infor
                            print element_infor
                        else:
                            for i_year in range(0,len(myItems1)):
                                cita_infor = cita_infor + '[' + myItems1[i_year]  + ';' + myItems2[i_year] + '];'
                                               
                            #------------------------------------------------------
                            # write the information          
                            element_infor = 'University:[' + university + ']; ' + 'Title:[' + paper_title + ']; ' + 'Year:[' + paper_year + '];' + '\n'
                            element_infor = element_infor   +  'Open Time:[' + time_open + ';0]; ' + 'URL:[' + paper_url + ']; ' + '\n'
                            element_infor = element_infor + cita_infor
                            print element_infor
                              
                        #------------------------------------------------------
                        # write the information
                        list_file = open(total_element_infor_list_path,'a')
                        list_file.writelines(element_infor +'\n')                  #'\n' create a new line, Note that on windows, it is '\n', on Mac it is '\r', on Unix, it is '\n'
                        list_file.close()
                   
                         
                    except:
                        print "Can not open this pages!"
            #-----------------------------------------------------
            time.sleep(k_time_sleep1*(0.95+0.1*random.random()))      # if fail to open, then wait a moment.
            elapsed = (time.clock() - start)
            time_reserve = time_reserve - elapsed
            if time_reserve < 0:
                time_reserve = 1.5*24*3600                # every 1.5 days sleep for 1.5 hour
